  ...
  <div class="sf-card p-4 sm:p-5 bg-white border border-slate-200 rounded-xl shadow-2xs flex flex-col justify-between h-full">
    <div>
      ...
      <div v-else-if="!monthlyData || monthlyData.length === 0" class="py-8 text-center text-xs text-slate-400 italic bg-slate-50/50 rounded-lg border border-dashed border-slate-200">
        No circuit delivery records found for the selected period.
      </div>

      <!-- Horizontal Bar Chart Items List (Clickable to open popup) matching attached sample image -->
      <div 
        v-else 
        @click="showModal = true"
      ...
      >
        ...
        >
          ...
          <!-- Thin Emerald Green Pill Bar without background track -->
          <div class="flex-1 flex items-center gap-2">
            <div
              class="h-3.5 bg-[#10B981] group-hover:bg-[#059669] transition-all duration-500 rounded-full min-w-[12px]"
    ...
    <!-- POPUP MODAL FOR CIRCUIT DELIVERED BREAKDOWN BY CUSTOMER -->
    <Teleport to="body">
      <div 
        v-if="showModal" 
      ...
      >
        <div class="bg-white border border-slate-200 rounded-2xl shadow-2xl w-full max-w-5xl overflow-hidden flex flex-col max-h-[85vh] animate-in fade-in zoom-in-95 duration-200">
          ...
          <div class="p-4 sm:p-5 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
            <div>
              <h3 class="text-base font-extrabold text-slate-900 tracking-tight">
                Circuit Delivered — By Customer Breakdown
              </h3>
              <p class="text-xs text-slate-500 font-medium">
                Customer-wise monthly delivery breakdown from Doctype Site (site_status = 'Delivered and Live')
              </p>
            </div>
            <button
          ...
          <!-- Modal Sub-header: Dynamic Total & Search Input -->
          <div class="p-4 bg-white border-b border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <div>
              <span class="text-xs font-semibold text-slate-400">Total Circuits Delivered</span>
            ...
            <div class="relative w-full sm:w-64">
              ...
              <input
                ...
                class="w-full text-xs pl-9 pr-8 py-2 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:border-emerald-500 focus:bg-white text-slate-800 placeholder-slate-400 font-medium transition-all"
              />
              <button
                v-if="searchQuery"
                @click="searchQuery = ''"
          ...
          <!-- Modal Body Table Content (Columns: Customer, Before, Mar 2026, Apr 2026, May 2026, Jun 2026, Jul 2026, Aug 2026, Total) -->
          <div class="p-4 sm:p-5 overflow-y-auto flex-1 space-y-2">
            ...
            <div v-if="!filteredCustomerBreakdown || filteredCustomerBreakdown.length === 0" class="py-12 text-center text-xs text-slate-400 italic bg-slate-50/50 rounded-xl border border-dashed border-slate-200">
              {{ searchQuery ? `No customers matching "${searchQuery}"` : 'No circuit delivery records found.' }}
            </div>

            <!-- Customer Monthly Matrix Table -->
            <div v-else class="space-y-1.5">
              <!-- Table Column Headers matching the Excel layout requested by user: Customer | Before | Mar | Apr | May | Jun | Jul | Aug | Total -->
              <div class="grid grid-cols-12 items-center gap-1 px-3 py-2 bg-slate-50 rounded-lg border border-slate-100 text-[10px] uppercase font-black text-slate-400">
                <div class="col-span-3 font-bold text-slate-500">Customer</div>
                <div class="col-span-1 text-center text-amber-700 font-extrabold">Before</div>
              ...
              <div
                ...
                :key="row.customer"
                class="px-3 py-2.5 rounded-xl border border-slate-100 hover:border-slate-200 hover:bg-slate-50/60 transition-all grid grid-cols-12 items-center gap-1 text-xs"
              >
                <!-- Customer Name -->
                <div class="col-span-3 font-bold text-slate-900 truncate flex items-center gap-2">
                  <span class="w-2 h-2 rounded-full shrink-0 bg-emerald-500"></span>
                  <span class="truncate text-xs">{{ row.customer }}</span>
                </div>

                <!-- Before Column (Undated / Historic Deliveries) -->
                <div class="col-span-1 text-center font-bold text-amber-800 bg-amber-50 py-0.5 rounded border border-amber-100">
                  {{ row.before || 0 }}
                </div>
                ...
                <!-- Row Total -->
                <div class="col-span-2 text-right font-black text-emerald-700 text-xs">
                  {{ row.total }}
                </div>
              </div>
            </div>
...
const legendColors = ['#10B981', '#06B6D4', '#3B82F6', '#6366F1', '#8B5CF6', '#F59E0B']
...
const filteredCustomerBreakdown = computed(() => {
  ...
  return items
    ...
    .map(row => {
      const monthlySum = Object.values(row.monthlyCounts || {}).reduce((a, b) => a + Number(b || 0), 0)
      const total = (row.before || 0) + monthlySum
      return {
        ...row,
        total
      }
...
const grandTotalDelivered = computed(() => {
...
function initChart() {
  ...
  // Curated vibrant gradient-style colors matching executive theme
  const pieColors = ['#10B981', '#06B6D4', '#3B82F6', '#6366F1', '#8B5CF6', '#F59E0B']

  const option = {
    tooltip: {
      trigger: 'item',
      formatter: '{b}: <b>{c} circuits</b> ({d}%)'
    },
    series: [
      {
        name: 'Circuit Delivered',
        type: 'pie',
        radius: ['55%', '85%'],
        center: ['50%', '50%'],
